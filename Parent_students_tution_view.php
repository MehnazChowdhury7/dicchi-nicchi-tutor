      ...
      {   
          //parent_name
           ...
           $output .= '  
                ...
                <tr>  
                    <td width="30%"><label>Student Subject</label></td>  
                    <td width="70%">'.$subject_name.'</td>  
                </tr>'; 
               
$output  .=     '<tr>  
                    <td width="30%"><label>Tuition days</label></td>
                    <td width="70%">'; 

                    foreach ($tution_days as $tution_day) {
                    $query_row_tution_days = "SELECT id,day_name FROM days Where id = '".$tution_day."' ";
                    $result_row_tution_days = mysqli_query($conn, $query_row_tution_days);

                        while($row_tution_days = mysqli_fetch_array($result_row_tution_days))
                        {
                            $day_id = $row_tution_days["id"];
                        
                            foreach ($tution_days as $tution_day) {
                                if($day_id == $tution_day){
                                    $output  .= $row_tution_days["day_name"].'<br>';
                                }
                            }
                        }                                           
                    }
...
$output  .=     '<tr>  
                    <td width="30%"><label>tution time </label></td>  
                    <td width="70%">From: '.$tution_time_from.' To: '.$tution_time_to.'</td>  
                </tr>
    ...
    $sql = "DELETE FROM student_info WHERE id = '".$_POST["id"]."'";  
    ...
    if($tution_status == 1){
         $sql = "UPDATE  student_info SET status ='0' where id='$student_id' ";
    }elseif ($tution_status == 0) {
         $sql = "UPDATE  student_info SET status ='1' where id='$student_id' ";
    }
