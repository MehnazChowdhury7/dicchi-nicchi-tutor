    ...
    session_start();
     ...
     if($_POST['value_a'] == "validParent"){
        ...
        if($code == 200 && !( curl_errno($handle)))
        {
            ...
            //insert payment
            if($conn){
                $sql = " INSERT INTO parent_tution_payment(id, tution_id, amount, transaction_id, transaction_date, transaction_type) VALUES ( '' , '$tution_id' , '$amount', '$tran_id', '$tran_date', '$transaction_type' ) ";                 
                $result = mysqli_query($conn , $sql);
                
                if($result != null){
                    $sql_parent_tution_payment = "SELECT * FROM parent_tution_payment WHERE  tution_id='$tution_id' ";
                    $result_parent_tution_payment = mysqli_query($conn , $sql_parent_tution_payment);
                    $row_parent_tution_payment = mysqli_fetch_array($result_parent_tution_payment);

                    $parent_payment_id = $row_parent_tution_payment['id'];
                    $tution_id = $row_parent_tution_payment['tution_id'];

                    $sql_tution_Upadet =  " UPDATE  tution SET parent_is_pay ='1' , parent_payment_id = '$parent_payment_id'  where id='$tution_id' ";
