<?php
	...
	if( ($_SESSION['abc'] == "validAdmin") || isset($_COOKIE['rm']) ){
	   ...
	   //all student
                $query = "SELECT * FROM tutorregistration ";  
 ...
 $(document).ready(function(){ 
    ...
    //delete data
      $(document).on('click', '.btn_delete', function(){  
           ...
           var action = "admin_teacher_delete";
     ...
     $(document).on('click', '.btn_status', function(){  
           var teacher_id=$(this).data("id5");
           var value=$(this).val();
           var action = "admin_teacher_status_change";
           if(confirm("Are you sure you want to change status ?"))  
           {  
                $.ajax({  
                     url:"Admin_account_verify.php",  
                     method:"POST",  
                     data:{teacher_id:teacher_id , action:action, value:value},  
                     dataType:"text",  
                     success:function(data){  
                          alert(data);
                          location.reload();  
     ...
     //status end 
       ...
       $(document).on('click', '.btn_verify', function(){  
           ...
           if(value == "account_approve"){
            ...
            var action = "Teacher Account Approve";
           }else if(value == "account_unapproved"){
            var teacher_id=$(this).data("id6");
            var admin_id=$('.admin_id').val();
            
            var action = "Teacher Account Unapproved";
           }
           ...
           {  
                $.ajax({  
                     url:"Admin_account_verify.php",  
                     method:"POST",  
                     data:{teacher_id:teacher_id , admin_id:admin_id , action:action},  
                     dataType:"text",  
                     success:function(data){  
                          alert(data);
                          location.reload();  
